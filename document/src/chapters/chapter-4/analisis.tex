...
Pada skenario ini, sistem memiliki ketersediaan tiket yang banyak sehingga penjualan tiket yang berhasil berjalan cukup lama. Pola ini ditunjukkan dengan jelas pada Gambar \ref{fig:pattern-stress-traffic}. Perhatikan bagaimana kurva ungu (POST /orders) tetap stabil lalu menurun saat tiket habis, sementara kurva abu-abu (GET /events/availability) melonjak tajam setelahnya. Ini menandakan fase di mana pengguna beralih dari aktif memesan menjadi hanya memeriksa sisa ketersediaan.
...
Meskipun secara teori ide pemartisian pada PostgreSQL dengan CitusData terdengar baik, terdapat \textit{tradeoff} dari sisi koordinator yang ternyata tidak dapat diabaikan. Pertukaran ini semakin tidak dapat diabaikan pada kasus penjualan tiket ketika pola kueri adalah banyak kueri ringan yang dijalankan secara berulang-ulang. Perbedaan penggunaan sumber daya antara koordinator dan worker CitusData terlihat jelas pada Gambar \ref{fig:citusdata-usage}. Kurva ungu, yang mewakili koordinator (cituscluster-0), mencapai puncak penggunaan CPU di atas 5 cores (rata-rata 2.1). Sebaliknya, kurva biru dan coklat yang mewakili worker (cituscluster-1 dan cituscluster-2) memiliki puncak yang jauh lebih rendah. Beban yang hampir dua kali lipat pada koordinator ini menyoroti overhead dari perencanaan dan distribusi kueri.
